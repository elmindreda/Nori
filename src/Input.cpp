...
Manager::Manager(GL::Context& initContext):
  context(initContext)
{
  ...
  context.getResizedSignal().connect(*this, Manager::sizeCallback);
...
bool Manager::isKeyDown(const Key& key) const
{
  int externalKey = key;
...
bool Manager::isButtonDown(unsigned int button) const
{
...
unsigned int Manager::getWidth(void) const
{
  return context.getWidth();
}

unsigned int Manager::getHeight(void) const
{
  return context.getHeight()t;
}

const Vector2& Manager::getCursorPosition(void) const
{
  int x, y;
  glfwGetMousePos(&x, &y);
...
void Manager::setCursorPosition(const Vector2& newPosition)
{
  cursorPosition = newPosition;
  glfwSetMousePos((int) newPosition.x, (int) newPosition.y);
}

SignalProxy2<void, unsigned int, unsigned int> Manager::getResizedSignal(void)
{
  return resizedSignal;
}

SignalProxy2<void, Key, bool> Manager::getKeyPressedSignal(void)
{
  return keyPressedSignal;
}

SignalProxy1<void, wchar_t> Manager::getCharInputSignal(void)
{
  return charInputSignal;
}

SignalProxy2<void, unsigned int, bool> Manager::getButtonClickedSignal(void)
{
  return buttonClickedSignal;
}

SignalProxy1<void, const Vector2&> Manager::getCursorMovedSignal(void)
{
  return cursorMovedSignal;
}

SignalProxy1<void, int> Manager::getWheelTurnedSignal(void)
{
...
void Manager::sizeCallback(unsigned int width, unsigned int height)
{
  resizedSignal.emit(width, height);
}

void Manager::keyboardCallback(int key, int action)
{
...
void Manager::characterCallback(int character, int action)
{
...
void Manager::mousePosCallback(int x, int y)
{
